...
public class SamplesController {
	...
	ResponseEntity helloSample(@RequestBody Person person) {
		HelloWorkflow workflow = client.newWorkflowStub(HelloWorkflow.class,
				WorkflowOptions.newBuilder().setTaskQueue("HelloSampleTaskQueue").setWorkflowId("HelloSample").build());
	...
	@PostMapping(value = "/update/purchase", consumes = { MediaType.APPLICATION_JSON_VALUE }, produces = {
			MediaType.TEXT_HTML_VALUE })
	ResponseEntity purchase(@RequestBody Purchase purchase) {
		PurchaseWorkflow workflow = client.newWorkflowStub(PurchaseWorkflow.class, WorkflowOptions.newBuilder()
				.setTaskQueue("UpdateSampleTaskQueue").setWorkflowId("NewPurchase").build());
	...
	/*
	 * @PostMapping(value = "/kafka", consumes = { MediaType.TEXT_PLAIN_VALUE },
	 * produces = { MediaType.TEXT_HTML_VALUE }) ResponseEntity
	 * sendToKafka(@RequestBody String message) { MessageWorkflow workflow =
	 * client.newWorkflowStub(MessageWorkflow.class, WorkflowOptions.newBuilder()
	 * .setTaskQueue("KafkaSampleTaskQueue").setWorkflowId("MessageSample").build())
	 * ;
	 * 
	 * WorkflowClient.start(workflow::start); workflow.update(message);
	 * 
	 * // wait till exec completes
	 * WorkflowStub.fromTyped(workflow).getResult(Void.class); // bypass thymeleaf,
	 * don't return template name just result return new
	 * ResponseEntity<>("\" Message workflow completed\"", HttpStatus.OK); }
	 */
	...
	@PostMapping(value = "/customize", consumes = { MediaType.APPLICATION_JSON_VALUE }, produces = {
			MediaType.TEXT_HTML_VALUE })
	ResponseEntity customizeSample() {
		CustomizeWorkflow workflow = client.newWorkflowStub(CustomizeWorkflow.class, WorkflowOptions.newBuilder()
				.setTaskQueue("CustomizeTaskQueue").setWorkflowId("CustomizeSample").build());
